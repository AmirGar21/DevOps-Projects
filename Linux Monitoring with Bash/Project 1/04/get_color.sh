...
col1_bg=$(grep "^column1_background=" config.conf | cut -d "=" -f 2)
if [[ -z "$col1_bg" ]]; then
    col1_bg=1
fi

col1_font=$(grep "^column1_font_color=" config.conf | cut -d "=" -f 2)
if [[ -z "$col1_font" ]]; then
    col1_font=4
fi

col2_bg=$(grep "^column2_background=" config.conf | cut -d "=" -f 2)
if [[ -z "$col2_bg" ]]; then
    col2_bg=4
fi

col2_font=$(grep "^column2_font_color=" config.conf | cut -d "=" -f 2)
if [[ -z "$col2_font" ]]; then
    col2_font=3
fi
